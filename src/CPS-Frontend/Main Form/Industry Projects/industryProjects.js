...
const IndustryProjectCompletion = () => {
  ...
  const handleIndustryAttachementsSelect = (index, event) => {
    ...
    newIndustryProjectsNumber[index] = {
      ...newIndustryProjectsNumber[index],
  ...
  return (
    <>
      ...
      >
        <CardHeader
          title="Successful completion of Industry Projects
identified by the institute. (No duplication
with UG & PG Projects mentioned earlier)"
        ...
        >
          <Typography>
            Have you ever done Successful completion of Industry Projects
            identified by the institute. (No duplication with UG & PG Projects
            mentioned earlier)?
          </Typography>
          <RadioGroup
            row
            sx={{ gap: 2 }}
            value={industryProjectsChecked}
            onChange={(e) => handleaIndustryProjectsChecked(e)}
          >
        ...
        {industryProjectsChecked !== "No" && industryProjectsChecked && (
          <>
            <FormControl fullWidth>
              <TextField
                label="How many Industry Projects have Completed Sucessfully?"
                type="number"
                variant="outlined"
                fullWidth
                onChange={(e) => {
                  ...
                  setNumberofIndustryProjects(e.target.value);
                }}
              />
            </FormControl>

            {industryProjectsChecked !== "No" &&
              IndustryProjectsNumber.map((_, index) => (
                <FormControl fullWidth sx={{ marginTop: "20px" }}>
                  <Typography
                    variant="h5"
                    sx={{ marginBottom: "20px", textAlign: "center" }}
                  >
                    Information About Industry Project {index + 1}
                  </Typography>
                  <FormControl fullWidth>
                    <InputLabel id="t">
                      Select State of Industry Project {index + 1}
                    ...
                    <Select
                      ...
                      value={_.IndustryProjectsOption}
                      onChange={(e) =>
