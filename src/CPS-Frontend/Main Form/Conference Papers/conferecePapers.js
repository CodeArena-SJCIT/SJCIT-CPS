...
const ConferencePapers = () => {
        ...
        if (newList.length < count) {
          newList.push(
            ...Array(count - newList.length).fill({
              conferencePaperMainGuideSelectOption: "",
            })
          );
        } else {
  ...
  const handleSelectedOptionForConferencePaperGuide = (index, option) => {
    ...
    newList[index] = {
      ...newList[index],
      conferencePaperMainGuideSelectOption: option,
    };
  ...
  return (
    <>
      ...
      >
        <CardHeader
          title="Conference papers indexed in SCI / Scopus / Web of Science Conference / any internationally renowned conference"
        ...
        <FormGroup sx={{ marginTop: "10px", marginBottom: "20px" }}>
          <FormControlLabel
            control={
              <Checkbox
                checked={conferencePaperChecked}
                onChange={(e) => handleConferencePaperChecked(e)}
              />
            }
            label="Do you have any Conference papers indexed in SCI / Scopus / Web of Science Conference / any internationally renowned conference?"
          />
        </FormGroup>
          ...
          <>
            <CardContent>
              <TextField
                ...
                fullWidth
                onChange={(e) => setnumberOfConferencePapers(e.target.value)}
              />
            </CardContent>
            ...
            {conferencePaperList.map((_, index) => (
              <>
                <FormGroup sx={{ marginTop: "40px", marginBottom: "20px" }}>
                  ...
                  >
                    Information About Conference Paper / Book Chapter{" "}
                    {index + 1}
                  </Typography>
                  <Typography>
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        checked={
                          _.conferencePaperMainGuideSelectOption === "Yes"
                        }
                        onChange={() =>
                          handleSelectedOptionForConferencePaperGuide(
                            index,
                            "Yes"
                          )
                        }
