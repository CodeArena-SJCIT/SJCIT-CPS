...
const JournalPapers = () => {
  ...
  const handleSelectedOptionForJournalPaperGuide = (index, option) => {
    ...
    newjournalPaperList[index] = {
      ...newjournalPaperList[index],
      journalPaperGuideSelectOption: option,
    };
  ...
  return (
    <>
      ...
      >
        <CardHeader
          title="Journal Papers / Book chapter in SCI / Scopus"
        ...
        <FormGroup sx={{ marginTop: "10px", marginBottom: "20px" }}>
          <FormControlLabel
            control={
              <Checkbox
                checked={journalPaperChecked}
                onChange={(e) => handlejournalPaperChecked(e)}
              />
            }
            label="Do you have any journal papers or book chapters published in SCI or Scopus-indexed journals?"
          />
        </FormGroup>
        ...
        {journalPaperChecked && (
          <>
            <CardContent>
              <TextField
                type="number"
                label="How many journal papers or book chapters published in SCI or Scopus-indexed journals?"
                fullWidth
                onChange={(e) => setnumberOfJournalPapers(e.target.value)}
              />
            </CardContent>
            ...
            {journalPaperList.map((_, index) => (
              <>
                <FormGroup sx={{ marginTop: "40px", marginBottom: "20px" }}>
                  ...
                  >
                    Information About Journal Paper / Book Chapter {index + 1}
                  </Typography>
                  <Typography>
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        checked={_.journalPaperGuideSelectOption === "Yes"}
                        onChange={() =>
                          handleSelectedOptionForJournalPaperGuide(index, "Yes")
                        }
                      />
                    }
                    label="Yes"
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        checked={_.journalPaperGuideSelectOption === "No"}
                        onChange={() =>
                          handleSelectedOptionForJournalPaperGuide(index, "No")
                        }
                      />
                    }
                    label="No"
                ...
                {_.journalPaperGuideSelectOption === "No" && (
                  <FormGroup sx={{ marginTop: "10px", marginBottom: "20px" }}>
                    <TextField
                      type="number"
                      label="How many Authors / Guides were there"
                    />
                  </FormGroup>
                )}
              </>
